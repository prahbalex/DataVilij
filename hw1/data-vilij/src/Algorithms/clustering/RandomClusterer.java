...
public class RandomClusterer extends Clusterer {
    ...
    public RandomClusterer(int k, DataSet dataset, int maxIterations, int updateInterval, boolean tocontinue, ApplicationTemplate applicationTemplate) {
        super(k);
        this.dataset = dataset;
        this.maxIterations = maxIterations;
        this.updateInterval = updateInterval;
...
//            recomputeCentroids();
    ...
    public void cluster(){
        ...
        for (String s: dataset.getLabels().keySet()
            ...
            int x = (int)(Math.random()*(numberOfClusters -1 + 1) +1);
            String a = Integer.toString(x);
            dataset.updateLabel(s, a);
        }
